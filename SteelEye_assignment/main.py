...
##listing
@app.get('/listing')
async def read_trades(page: int,page_rate: Optional[int] = 10, sort_by: Optional[str] = None, isdesc : Optional[bool] = False ):
    page = page - 1
    ...
    if sort_by:
        ...
        if isinstance(result_trades,str):
            return {"status": "failure","message": "Oops! Invalid sortby property name", "sortbyReceived": sort_by, "sortbyList": sort_by_list}, 400
...
async def read_trade(order_Id : str):
    result = []
...
async def search_trade(page:int,search:str,page_rate : Optional[str] = 10,case_sensitive : Optional[bool] = False,sort_by : Optional[str] = None, isdesc: Optional[bool] = False):
    ...
    if sort_by:
        ...
        if isinstance(searchResults, str):
            return {"status": "failure","message": "Invalid sortby property name", "sortbyReceived": sort_by, "sortbyList": sort_by_list}, 400
...
@app.get("/filter")
async def filter_trades(page: int, page_rate: Optional[int]=10, asset_class: Optional[str] = None, max_price: Optional[float] = None, min_price: Optional[float] = None, trade_type: Optional[str] = None, sort_by: Optional[str] = None, isdesc: Optional[bool] = False):
    page = page- 1
    if page_rate <= 0:
        return {"status": "failure", "message": "Oops! Invalid page rate"}, 400
